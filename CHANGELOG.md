...
- Add `'volume_map'` boundary type to `SolverSPH` for plane / box / sphere shapes (Bender et al. 2020 IEEE TVCG): precomputed half-space V_B(d) table, boundary pressure (Eq. 15) with outward-direction kernel-gradient fix and non-negative pressure mirror for explicit-WCSPH stability, and matrix-free implicit boundary-friction CG with sticky/sliding variants exposed via `Config.boundary_friction_viscosity` and `Config.boundary_sticky`
- Add `'volume_map'` boundary type to `SolverSPH` for plane / box / sphere shapes (Bender et al. 2020 IEEE TVCG): precomputed half-space V_B(d) table, boundary pressure (Eq. 15) with outward-direction kernel-gradient fix and non-negative pressure mirror for explicit-WCSPH stability, and matrix-free implicit boundary-friction CG with sticky/sliding variants. New Config fields: `boundary_friction_viscosity`, `boundary_sticky`, `viscosity_solver_max_iter`, `viscosity_solver_tol`, `volume_map_table_size`
- Add `'volume_map'` boundary type to `SolverSPH` for plane / box / sphere shapes: precomputed half-space V_B(d) table from Bender et al. 2020 (IEEE TVCG), Adami-style mirror-pressure boundary force (`p_b = max(p_i, 0) + ρ_i max(0, -g·n) max(0, -d)`, linear in pressure for explicit-WCSPH stability) with V_B-scaled coefficient and damped-spring escapee-recovery fallback for `d < 0`, plus matrix-free implicit boundary-friction CG with sticky/sliding variants (Bender Eqs. 16-19). New Config fields: `boundary_friction_viscosity`, `boundary_sticky`, `viscosity_solver_max_iter`, `viscosity_solver_tol`, `volume_map_table_size`, `volume_map_wall_stiffness`, `volume_map_wall_damping`. Known limitation: closed-container geometries (e.g. six-plane box) trigger explicit-spring ringing between opposing walls; design target is column-on-plane granular flow
