...
- Add optional first-order kernel-gradient correction (Bonet-Lok / MLS) to `SolverSPH` via `Config.kernel_gradient_correction` (default `"none"`, alternative `"mls"`) and singular-fallback threshold `kernel_correction_det_floor`. Restores velocity-gradient consistency under particle disorder; stress-force correction deferred to preserve momentum conservation.
