...
- Dual-namespace model: per-particle **material** parameters on `Model.sph.*` (`young_modulus`, `poisson_ratio`, `friction`, `cohesion`, `dilatancy`, `viscosity`, `yield_pressure`, `particle_type`, `wall_normal`); per-particle **evolving** fields on `State.sph.*` (`density`, `pressure`, `plastic_strain`, `stress`, `strain_rate`, `velocity_gradient`). New attributes must be registered in `SolverSPH.register_custom_attributes()`.
...
- Neighbor search is `wp.HashGrid` (3D) with `support_radius = support_radius_factor · h`, rebuilt every step. Density is computed by direct summation; there is no continuity equation, Shepard filter, or δ-SPH term.
...
- Known dead / unwired config knobs: `kernel_type`, `dilatancy`, `viscous_damping`, `yield_pressure`. Either wire them before exposing a new one, or prune — do not add more silently-ignored options.
